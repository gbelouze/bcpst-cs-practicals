...
\commentaire{
    ...
    De manière générale, lorsque l'on écrit une fonction récursive, il y a une partie qui assure la \textit{terminaison}, c'est-à-dire qui s'assure que la fonction va finir par arêter de s'appeler elle-même, et une partie qui "fait le travail". Ici, le test \mintinline{python}{if n == 0} est là pour assurer la terminaison. Implicitement, on suppose que l'utilisateur.ice n'utilise \texttt{factoriel} qu'avec des entiers \mintinline{python}{n >= 0}. Que se passe-t-il si on appel \mintinline{python}{factoriel(-1)} ? \mintinline{python}{factoriel(1.5)} ?
}
